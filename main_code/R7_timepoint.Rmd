...
ootpm <- read.csv("/Users/lubeifang/Desktop/Kleb_dap/files/timepoint_tpm.csv")
...
change.trend.df <- read.csv("/Users/lubeifang/Desktop/SD7_56TFchangeTYPE.csv")
...
gene_tpm <- anti_join(tpm,TF_tpm,by="X")

df <- rbind (data.frame(cate = "TF", num = TF_tpm$TP24),
             data.frame(cate = "other_gene", num = gene_tpm$TP24))
...
# ggsave("/Users/lubeifang/Desktop/violin.pdf",width = 4, height = 6)
...
# 2 means up, 1 means no change, 0 means dwon
change.df$cc1 <- ifelse(change.df$c1>=2, "2", ifelse(change.df$c1 <=-2, "0","1"))
change.df$cc2 <- ifelse(change.df$c2>=2, "2", ifelse(change.df$c2 <=-2, "0","1"))
change.df$cc3 <- ifelse(change.df$c3>=2, "2", ifelse(change.df$c3 <=-2, "0","1"))
change.df$cc <- paste(change.df$cc1, change.df$cc2, change.df$cc3)

type <- as.data.frame(table(change.df$cc))
type$Type <- c(1,2,3,0,4,5,6,7,8)

change.df <- merge(change.df,type,by.x="cc",by.y="Var1")
# write.csv(change.df,"/Users/lubeifang/Desktop/SD7.csv",row.names = F)
```
...
change.df
tfinC
TF_type_cluster <- merge(tfinC,change.df,by="Gene",all.x = T)
