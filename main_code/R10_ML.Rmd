...
########come on PEAKS!!!###########
# load annotation
hvkp4_txdb <- txdbmaker::makeTxDbFromGFF("/Users/lubeifang/Desktop/BIOTOOLS/ref/CHIPPEAKANNO/hvkp4.gff")
...
ggsave("/Users/lubeifang/Desktop/Kleb_dap/Figure/Figure7/96TF_bar.pdf", width = 18, height = 6)
```
...
```{r}
library(patchwork)
training_history <- read.csv("/Users/lubeifang/Desktop/training_history_96TF_test0824.csv")

p1<- ggplot(training_history)+
  geom_line(aes(x=epoch,y=train_loss,color="trainL"))+
  geom_line(aes(x=epoch,y=val_loss,color="valL")) +
  scale_color_manual(values = c("trainL"="lightgreen","valL"="lightblue")) +
  theme_bw()

p2<-ggplot(training_history)+
  geom_line(aes(x=epoch,y=val_auc,color="auc"))+
  geom_line(aes(x=epoch,y=val_auprc,color="auprc")) +
  scale_color_manual(values = c("auc"="pink","auprc"="orange")) +
  theme_bw()
p <- p1+p2
ggsave("/Users/lubeifang/Desktop/Kleb_dap/Figure/Figure7/train.pdf", p, width = 10, height = 3)

test_matrix <- read.csv("/Users/lubeifang/Desktop/test_matrix_96TF_test0824.csv")

ggplot(test_matrix,aes(x=Predictions,group = as.factor(Labels),fill= as.factor(Labels)))+
...
```
```{R}
off_limits_list <- data.frame()
peak <- read.csv("/Users/lubeifang/Desktop/Kleb_dap/ML/tf_bind/data/Trail4/allpeak_96TF.csv",header = F)
for (i in unique(peak$V1)){
  print(i)
  peaks <- peak[peak$V1 == i,]
  col_names <- c("TF","chr", "start", "end", "peak_name", "score",
                 "strand", "signalValue", "-log10pvalue", "-log10qvalue", 
                 "relativesummitpositiontopeakstart")
  colnames(peaks) <- col_names
  ...
  start(gR) < 1 |
    ...
    if (length(off_limits) >0){
      temp <- data.frame(TF = i, off_limits = length(off_limits))
      off_limits_list <- rbind(off_limits_list, temp)
      gR<- gR[-off_limits]
    }
    sequences <- get_sequence(gR, kp_stringset)
    writeXStringSet(sequences, paste0("/Users/lubeifang/Desktop/Kleb_dap/ML/tf_bind/data/Trail4/peak_fasta/",i,".fasta"), append=FALSE,
                    compress=FALSE, compression_level=NA, format="fasta")
  
}

# peaks <- peak[peak$V1 == "RS22030",]
#   col_names <- c("TF","chr", "start", "end", "peak_name", "score",
#                  "strand", "signalValue", "-log10pvalue", "-log10qvalue", 
#                  "relativesummitpositiontopeakstart")
#   colnames(peaks) <- col_names
...
peak <- peak[peak$V6 != "A8-1_L6_peak_1446" & peak$V6 != "D20-1_L4_peak_2940" & peak$V6 != "J13-1_L4_peak_404",]
write.csv(peak,"/Users/lubeifang/Desktop/Kleb_dap/ML/tf_bind/data/Trail4/allpeak_96TF_noofflimit.csv",row.names = F)
